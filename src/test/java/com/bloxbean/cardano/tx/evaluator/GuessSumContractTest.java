...
public class GuessSumContractTest  {
    ...
    public void invokeContract() throws Exception {
        ...
        TxBuilder contractTxBuilder = output.outputBuilder()
                ...
                .andThen((context, txn) -> {
                    ...
                    //Evaluate ExUnits
                    TxEvaluator txEvaluator = new TxEvaluator(context.getUtxos(), new SlotConfig(1000, 0, 100));
                    List<Redeemer> redeemerList = txEvaluator.evaluateTx(txn, costMdls);
...
//        int refScriptOutputIndex = 0;
    ...
    private ExUnits evaluateExUnits(Transaction transaction) throws ApiException, CborSerializationException {
        Result<List<EvaluationResult>> evalResults = backendService.getTransactionService().evaluateTx(transaction.serialize());
